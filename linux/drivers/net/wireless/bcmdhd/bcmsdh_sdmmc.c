...
sdioh_attach(osl_t *osh, struct sdio_func *func)
{
	...
	DHD_DBGMSG("%s: set sd_f2_blocksize %d\n", __FUNCTION__, sd_f2_blocksize);
...
sdioh_cis_read(sdioh_info_t *sd, uint func, uint8 *cisd, uint32 length)
{
	...
	DHD_DBGMSG("%s: func_cis_ptr[%d]=0x%04x\n", __FUNCTION__, func, sd->func_cis_ptr[func]);
...
static void IRQHandler(struct sdio_func *func)
{
	...
	if (mutex_is_locked(&sd->claim_host_mutex)) {
		DHD_DBGMSG("%s: muxtex is locked and return\n", __FUNCTION__);
		return;
	}
...
#if defined(MMC_SW_RESET) && LINUX_VERSION_CODE >= KERNEL_VERSION(4, 18, 0)
	/* MMC_SW_RESET */
	DHD_DBGMSG("%s: call mmc_sw_reset\n", __FUNCTION__);
	sdio_claim_host(sd->func[0]);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 0)
...
#elif defined(MMC_HW_RESET) && LINUX_VERSION_CODE >= KERNEL_VERSION(4, 2, 0)
	/* MMC_HW_RESET */
	DHD_DBGMSG("%s: call mmc_hw_reset\n", __FUNCTION__);
	sdio_claim_host(sd->func[0]);
