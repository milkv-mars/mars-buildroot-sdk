...
dhd_wlan_set_power(int on, wifi_adapter_info_t *adapter)
{
	...
	if (on) {
		DHD_DBGMSG("======== PULL WL_REG_ON(%d) HIGH! ========\n", gpio_wl_reg_on);
		if (gpio_wl_reg_on >= 0) {
			err = gpio_direction_output(gpio_wl_reg_on, 1);
			if (err) {
				DHD_DBGMSG("%s: WL_REG_ON didn't output high\n", __FUNCTION__);
				return -EIO;
			}
		}
		...
		if (adapter->sdio_func && adapter->sdio_func->card && adapter->sdio_func->card->host) {
			mdelay(100);
			DHD_DBGMSG("======== mmc_power_restore_host! ========\n");
		...
		if (adapter->pci_dev) {
			mdelay(100);
			DHD_DBGMSG("======== pci_set_power_state PCI_D0! ========\n");
			...
			if (err < 0)
				DHD_DBGMSG("%s: PCI enable device failed", __FUNCTION__);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 32) && LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 0)
		if (adapter->sdio_func && adapter->sdio_func->card && adapter->sdio_func->card->host) {
			DHD_DBGMSG("======== mmc_power_save_host! ========\n");
			mmc_power_save_host(adapter->sdio_func->card->host);
		}
#endif
#elif defined(BCMPCIE)
		if (adapter->pci_dev) {
			DHD_DBGMSG("======== pci_set_power_state PCI_D3hot! ========\n");
...
#endif /* BCMPCIE */
#endif /* BUS_POWER_RESTORE */
		DHD_DBGMSG("======== PULL WL_REG_ON(%d) LOW! ========\n", gpio_wl_reg_on);
		if (gpio_wl_reg_on >= 0) {
			err = gpio_direction_output(gpio_wl_reg_on, 0);
			if (err) {
				DHD_DBGMSG("%s: WL_REG_ON didn't output low\n", __FUNCTION__);
				return -EIO;
			}
		}
	}
...
dhd_wlan_init_gpio(wifi_adapter_info_t *adapter)
{
	...
	*/
	...
	if (adapter->pdev) {
		...
		DHD_DBGMSG("%s: adapter->pdev is NULL\n", __FUNCTION__);
		return -1;
	}
#else
	...
	if (gpio_wl_host_wake >= 0) {
		...
		if (err < 0) {
			DHD_DBGMSG("%s: gpio_request(%d) for WL_HOST_WAKE failed %d\n",
		...
		if (err < 0) {
			DHD_DBGMSG("%s: gpio_direction_input(%d) for WL_HOST_WAKE failed %d\n",
				__FUNCTION__, gpio_wl_host_wake, err);
		...
		if (host_oob_irq < 0) {
			DHD_DBGMSG("%s: gpio_to_irq(%d) for WL_HOST_WAKE failed %d\n",
	...
	DHD_DBGMSG("%s: WL_HOST_WAKE=%d, oob_irq=%d, oob_irq_flags=0x%x\n", __FUNCTION__,
		gpio_wl_host_wake, host_oob_irq, host_oob_irq_flags);
#endif /* CUSTOMER_OOB */
	DHD_DBGMSG("%s: WL_REG_ON=%d\n", __FUNCTION__, gpio_wl_reg_on);
...
dhd_wlan_deinit_gpio(wifi_adapter_info_t *adapter)
{
	...
	if (gpio_wl_reg_on >= 0) {
		DHD_DBGMSG("%s: gpio_free(WL_REG_ON %d)\n", __FUNCTION__, gpio_wl_reg_on);
		gpio_free(gpio_wl_reg_on);
		gpio_wl_reg_on = -1;
	}
#ifdef CUSTOMER_OOB
	if (gpio_wl_host_wake >= 0) {
		DHD_DBGMSG("%s: gpio_free(WL_HOST_WAKE %d)\n", __FUNCTION__, gpio_wl_host_wake);
		gpio_free(gpio_wl_host_wake);
		gpio_wl_host_wake = -1;
	}
#endif /* CUSTOMER_OOB */
...
#endif
	DHD_DBGMSG("bus_type=%d, bus_num=%d, slot_num=%d\n",
